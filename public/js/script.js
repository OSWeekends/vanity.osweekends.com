...
(() => {
    ...
    function ajaxRequest(url, cb) {
        ...
        request.onreadystatechange = function() {
            if (request.readyState === 4) {
                ...
                request.status === 200 ? cb(false, JSON.parse(response)) : cb(true);
            }
        };
    ...
    function renderTeam(team){
        return team.map(member => {
            return `<a href="${member.twitter}" target="_blank" alt="${member.name} in Twitter">${member.name}</a>`;
        }).join(''); 
    }
    ...
    function renderCommunities(communities){
        return communities.map(community => {
            return `<a href="${community.url}" target="_blank" alt="${community.name} website">${community.name}</a>`;
        }).join('');
    }
    ...
    function renderSponsors(sponsors){
        return sponsors.map( sponsor => {
            return `<a href="${sponsor.url}" target="_blank" alt="${sponsor.name} website">${sponsor.name}</a>`;
        }).join('');
    }
    ...
    function renderTwitter(data){
        return `<h5 class="card-title">
            <a href="https://twitter.com/os_weekends" target="_blank" alt="OSW Twitter account">Twitter</a>
        </h5>
    ...
    function renderSlack(data){
        return `<h5 class="card-title">
            <a href="https://invitations-osweekends.herokuapp.com/" target="_blank" alt="OSW Slack invitation">Slack</a>
        </h5>
    ...
    function renderMeetup(data){
        return `<h5 class="card-title">
            <a href="https://www.meetup.com/es-ES/Open-Source-Weekends/" target="_blank" alt="OSW MeetUp account">MeetUp</a>
        </h5>
    ...
    function renderGithub(data){
        return `<h5 class="card-title">
            <a href="https://github.com/OSWeekends" target="_blank" alt="OSW GitHub account">GitHub</a>
        </h5>
    ...
    */
    function activityDetails(activity){
        ...
        const createdAt = moment(activity.created_at).fromNow();
    ...
    */
    function renderLatestActivity(activities){
        return activities.map(activityDetails).join('');
    }
